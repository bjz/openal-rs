...
impl Context {
	...
	pub fn as_raw(&self) -> *mut sys::ALCcontext {
		self.0.ctx
	}
	...
	pub fn using_source_distance_model(&self) -> bool {
		...
		(|| -> AltoResult<_> {
			unsafe {
				Ok(self.0.dev.0.alto.0.api.alIsEnabled(
					self.0
						.exts
	...
	pub fn use_source_distance_model(&self, value: bool) -> AltoResult<()> {
		...
		if value {
			unsafe {
				self.0.dev.0.alto.0.api.alEnable(
					self.0
						.exts
						.AL_EXT_source_distance_model()?
						.AL_SOURCE_DISTANCE_MODEL?,
			...
			unsafe {
				self.0.dev.0.alto.0.api.alDisable(
					self.0
						.exts
						.AL_EXT_source_distance_model()?
						.AL_SOURCE_DISTANCE_MODEL?,
	...
	pub fn set_doppler_factor(&self, value: f32) -> AltoResult<()> {
		let _lock = self.make_current(true);
		unsafe {
			self.0.dev.0.alto.0.api.alDopplerFactor(value);
		}
		self.get_error()
	}
	...
	pub fn set_speed_of_sound(&self, value: f32) -> AltoResult<()> {
		let _lock = self.make_current(true);
		unsafe {
			self.0.dev.0.alto.0.api.alSpeedOfSound(value);
		}
		self.get_error()
	}
	...
	pub fn gain(&self) -> f32 {
		...
		unsafe {
			self.0
				...
				.0
				.api
				.alGetListenerf(sys::AL_GAIN, &mut value);
		}
		value
	}
	/// `alListenerf(AL_GAIN)`
	pub fn set_gain(&self, value: f32) -> AltoResult<()> {
		let _lock = self.make_current(true);
		unsafe {
			self.0.dev.0.alto.0.api.alListenerf(sys::AL_GAIN, value);
		}
		self.get_error()
	}
	...
	pub fn position<V: From<[f32; 3]>>(&self) -> V {
		...
		unsafe {
			self.0.dev.0.alto.0.api.alGetListenerfv(
				sys::AL_POSITION,
				&mut value as *mut [f32; 3] as *mut sys::ALfloat,
			);
		}
		value.into()
	}
	...
	pub fn set_position<V: Into<[f32; 3]>>(&self, value: V) -> AltoResult<()> {
		...
		unsafe {
			self.0.dev.0.alto.0.api.alListenerfv(
				sys::AL_POSITION,
				&value as *const [f32; 3] as *const sys::ALfloat,
			);
		}
		self.get_error()
	}
	...
	pub fn orientation<V: From<[f32; 3]>>(&self) -> (V, V) {
		...
		unsafe {
			self.0.dev.0.alto.0.api.alGetListenerfv(
				sys::AL_ORIENTATION,
				&mut value as *mut [[f32; 3]; 2] as *mut sys::ALfloat,
			);
		}
	...
	pub fn set_orientation<V: Into<[f32; 3]>>(&self, value: (V, V)) -> AltoResult<()> {
		...
		unsafe {
			self.0.dev.0.alto.0.api.alListenerfv(
				sys::AL_ORIENTATION,
				&value as *const [[f32; 3]; 2] as *const sys::ALfloat,
			);
		}
		self.get_error()
	}
	...
	pub fn soft_gain_limit(&self) -> AltoResult<f32> {
		...
		Ok(unsafe {
			self.0
				...
				.0
				.api
				.alGetFloat(asgce.AL_GAIN_LIMIT_SOFT?)
		})
	}
		...
		let _ = (|| -> AltoResult<_> {
			...
			let value = unsafe {
				self.0
					...
					.0
					.api
					.alGetInteger(assr.AL_NUM_RESAMPLERS_SOFT?)
			};
		...
		(|| -> AltoResult<_> {
			...
			unsafe {
				self.0
					...
					.0
					.api
					.alGetListenerf(efx.AL_METERS_PER_UNIT?, &mut value);
			}
			Ok(value)
		})()
		.unwrap_or(1.0)
	...
	pub fn set_meters_per_unit(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.0
				...
				.0
				.api
				.alListenerf(efx.AL_METERS_PER_UNIT?, value);
		}
		self.get_error()
	}
	...
	fn make_raw_current(dev: &Device, ctx: *mut sys::ALCcontext) -> Option<MutexGuard<()>> {
		if let Ok(&ext::ALC_EXT_thread_local_context {
			alcSetThreadContext: Ok(astc),
			..
		}) = dev.0.alto.0.exts.ALC_EXT_thread_local_context()
		{
			unsafe {
				astc(ctx);
			}
			...
			unsafe {
				dev.0.alto.0.api.alcMakeContextCurrent(ctx);
			}
...
impl Drop for ContextInner {
	fn drop(&mut self) {
		...
		unsafe {
			self.dev.0.alto.0.api.alcDestroyContext(self.ctx);
		}
	}
}
	...
	fn drop(&mut self) {
		let _ = (|| -> AltoResult<_> {
			let apus = self
				...
				.and_then(|asdu| asdu.alProcessUpdatesSOFT)?;
...
impl Buffer {
	pub(crate) fn new<F: SampleFrame, B: AsBufferData<F>>(
		ctx: Context,
		data: B,
		freq: i32,
	) -> AltoResult<Buffer> {
		let mut buf = 0;
		{
			...
			unsafe {
				ctx.0
					...
					.0
					.api
					.alGenBuffers(1, &mut buf as *mut sys::ALuint);
			}
			ctx.get_error()?;
		}
		let mut buf = Buffer {
	...
	pub fn set_data<F: SampleFrame, B: AsBufferData<F>>(
		...
		if (sys::ALsizei::max_value() as usize) < size {
	...
	pub fn frequency(&self) -> sys::ALint {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetBufferi(self.buf, sys::AL_FREQUENCY, &mut value);
		}
		value
	}
	...
	pub fn bits(&self) -> sys::ALint {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetBufferi(self.buf, sys::AL_BITS, &mut value);
		}
		value
	}
	...
	pub fn channels(&self) -> sys::ALint {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetBufferi(self.buf, sys::AL_CHANNELS, &mut value);
		}
		value
	}
	...
	pub fn size(&self) -> sys::ALint {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetBufferi(self.buf, sys::AL_SIZE, &mut value);
		}
		value
	}
	...
	pub fn soft_loop_points(&self) -> (sys::ALint, sys::ALint) {
		...
		(|| -> AltoResult<_> {
			...
			unsafe {
				self.ctx.0.dev.0.alto.0.api.alGetBufferiv(
					self.buf,
					self.ctx.0.exts.AL_SOFT_loop_points()?.AL_LOOP_POINTS_SOFT?,
					&mut value as *mut [sys::ALint; 2] as *mut sys::ALint,
				)
			}
			Ok((value[0], value[1]))
		})()
	...
	pub fn set_soft_loop_points(&self, value: (sys::ALint, sys::ALint)) -> AltoResult<()> {
		let _lock = self.ctx.make_current(true);
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alBufferiv(
				self.buf,
				self.ctx.0.exts.AL_SOFT_loop_points()?.AL_LOOP_POINTS_SOFT?,
				&[value.0, value.1] as *const [sys::ALint; 2] as *const sys::ALint,
			);
		}
		self.ctx.get_error()
	}
	...
	fn drop(&mut self) {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alDeleteBuffers(1, &mut self.buf as *mut sys::ALuint);
		}
	}
}
...
impl SourceInner {
	...
	fn state(&self) -> SourceState {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcei(self.src, sys::AL_SOURCE_STATE, &mut value);
		}
		match value {
	...
	fn play(&self) {
		let _lock = self.ctx.make_current(true);
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alSourcePlay(self.src);
		}
	}
	fn pause(&self) {
		let _lock = self.ctx.make_current(true);
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alSourcePause(self.src);
		}
	}
	fn stop(&self) {
		let _lock = self.ctx.make_current(true);
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alSourceStop(self.src);
		}
	}
	fn rewind(&self) {
		let _lock = self.ctx.make_current(true);
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alSourceRewind(self.src);
		}
	}
	...
	fn relative(&self) -> bool {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcei(self.src, sys::AL_SOURCE_RELATIVE, &mut value);
		}
	...
	fn set_relative(&self, value: bool) {
		let _lock = self.ctx.make_current(true);
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alSourcei(
				self.src,
				sys::AL_SOURCE_RELATIVE,
	...
	fn gain(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_GAIN, &mut value);
		}
		value
	}
	fn set_gain(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_GAIN, value);
		}
		self.ctx.get_error()
	}
	...
	fn min_gain(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_MIN_GAIN, &mut value);
		}
		value
	}
	fn set_min_gain(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_MIN_GAIN, value);
		}
		self.ctx.get_error()
	}
	...
	fn max_gain(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_MAX_GAIN, &mut value);
		}
		value
	}
	fn set_max_gain(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_MAX_GAIN, value);
		}
		self.ctx.get_error()
	}
	...
	fn reference_distance(&self) -> f32 {
		...
		let mut value = 0.0;
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alGetSourcef(
				self.src,
				sys::AL_REFERENCE_DISTANCE,
				&mut value,
			);
		}
		value
	...
	fn set_reference_distance(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_REFERENCE_DISTANCE, value);
		}
		self.ctx.get_error()
	}
	...
	fn rolloff_factor(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_ROLLOFF_FACTOR, &mut value);
		}
		value
	}
	fn set_rolloff_factor(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_ROLLOFF_FACTOR, value);
		}
		self.ctx.get_error()
	}
	...
	fn max_distance(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_MAX_DISTANCE, &mut value);
		}
		value
	}
	fn set_max_distance(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_MAX_DISTANCE, value);
		}
		self.ctx.get_error()
	}
	...
	fn pitch(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_PITCH, &mut value);
		}
		value
	}
	fn set_pitch(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_PITCH, value);
		}
		self.ctx.get_error()
	}
	...
	fn position<V: From<[f32; 3]>>(&self) -> V {
		...
		let mut value = [0.0, 0.0, 0.0];
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alGetSourcefv(
				self.src,
				sys::AL_POSITION,
				&mut value as *mut [f32; 3] as *mut sys::ALfloat,
			);
	...
	fn set_position<V: Into<[f32; 3]>>(&self, value: V) -> AltoResult<()> {
		...
		let value = value.into();
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alSourcefv(
				self.src,
				sys::AL_POSITION,
				&value as *const [f32; 3] as *const sys::ALfloat,
			);
	...
	fn set_cone_inner_angle(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_CONE_INNER_ANGLE, value);
		}
		self.ctx.get_error()
	}
	...
	fn set_cone_outer_angle(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_CONE_OUTER_ANGLE, value);
		}
		self.ctx.get_error()
	}
	...
	fn cone_outer_gain(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_CONE_OUTER_GAIN, &mut value);
		}
		value
	}
	fn set_cone_outer_gain(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_CONE_OUTER_GAIN, value);
		}
		self.ctx.get_error()
	}
	...
	fn sec_offset(&self) -> f32 {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcef(self.src, sys::AL_SEC_OFFSET, &mut value);
		}
		value
	}
	fn set_sec_offset(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, sys::AL_SEC_OFFSET, value);
		}
		self.ctx.get_error()
	}
	...
	fn sample_offset(&self) -> sys::ALint {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcei(self.src, sys::AL_SAMPLE_OFFSET, &mut value);
		}
		value
	}
	fn set_sample_offset(&self, value: sys::ALint) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcei(self.src, sys::AL_SAMPLE_OFFSET, value);
		}
		self.ctx.get_error()
	}
	...
	fn byte_offset(&self) -> sys::ALint {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alGetSourcei(self.src, sys::AL_BYTE_OFFSET, &mut value);
		}
		value
	}
	fn set_byte_offset(&self, value: sys::ALint) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcei(self.src, sys::AL_BYTE_OFFSET, value);
		}
		self.ctx.get_error()
	}
	...
	fn soft_sec_offset_latency(&self) -> AltoResult<(f64, f64)> {
		...
		unsafe {
			assl.alGetSourcedvSOFT?(
				self.src,
				assl.AL_SEC_OFFSET_LATENCY_SOFT?,
				&mut value as *mut [f64; 2] as *mut f64,
			);
		}
	...
	fn soft_sample_offset_frac_latency(&self) -> AltoResult<(i32, i32, i64)> {
		...
		let mut value = [0, 0];
		unsafe {
			assl.alGetSourcei64vSOFT?(
				self.src,
				assl.AL_SAMPLE_OFFSET_LATENCY_SOFT?,
				&mut value as *mut [i64; 2] as *mut i64,
			);
		}
		self.ctx
			.get_error()
			.map(|_| ((value[0] >> 32) as i32, value[0] as i32, value[1]))
	}
	...
	fn soft_direct_channels(&self) -> bool {
		...
		(|| -> AltoResult<_> {
			let mut value = 0;
			unsafe {
				self.ctx.0.dev.0.alto.0.api.alGetSourcei(
					self.src,
					self.ctx
						.0
						.exts
		...
		.unwrap_or(sys::AL_FALSE as sys::ALint)
			== sys::AL_TRUE as sys::ALint
	}
	...
	fn soft_spatialization(&self) -> SoftSourceSpatialization {
		(|| -> AltoResult<_> {
			...
			unsafe {
				self.ctx.0.dev.0.alto.0.api.alGetSourcei(
					self.src,
					assp.AL_SOURCE_SPATIALIZE_SOFT?,
					&mut value,
				);
			}
	...
	fn soft_resampler(&self) -> AltoResult<sys::ALint> {
		...
		let mut value = 0;
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alGetSourceiv(
				self.src,
				assr.AL_SOURCE_RESAMPLER_SOFT?,
				&mut value,
			);
		}
		Ok(value.into())
	...
	fn set_soft_resampler(&self, value: sys::ALint) -> AltoResult<()> {
		...
		unsafe {
			let value = value.into();
			self.ctx.0.dev.0.alto.0.api.alSourceiv(
				self.src,
				assr.AL_SOURCE_RESAMPLER_SOFT?,
				&value,
			);
	...
	fn stereo_angles<V: From<[f32; 2]>>(&self) -> AltoResult<V> {
		...
		unsafe {
			self.ctx.0.dev.0.alto.0.api.alGetSourcefv(
				self.src,
				aesa.AL_STEREO_ANGLES?,
				&mut value as *mut [f32; 2] as *mut f32,
			);
		}
		Ok(value.into())
	}
	fn set_stereo_angles<V: Into<[f32; 2]>>(&self, value: V) -> AltoResult<()> {
		...
		unsafe {
			...
			self.ctx.0.dev.0.alto.0.api.alSourcefv(
				self.src,
				aesa.AL_STEREO_ANGLES?,
				&value as *const [f32; 2] as *const f32,
			);
		}
		self.ctx.get_error()
	}
	...
	fn set_radius(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			...
			self.ctx
				...
				.0
				.api
				.alSourcefv(self.src, aesr.AL_SOURCE_RADIUS?, &value);
		}
		self.ctx.get_error()
	}
		...
		let _ = (|| -> AltoResult<_> {
			...
			unsafe {
				self.ctx
					...
					.0
					.api
					.alSourcei(self.src, efx.AL_DIRECT_FILTER?, 0);
			}
			Ok(())
		})();
	}
	...
	fn set_aux_send_impl(
		...
		unsafe {
			arc_self.ctx.0.dev.0.alto.0.api.alSourceiv(
				arc_self.src,
				efx.AL_AUXILIARY_SEND_FILTER?,
				&mut [slot.as_raw() as sys::ALint, send, filter as sys::ALint]
					as *mut [sys::ALint; 3] as *mut sys::ALint,
			);
		}
		...
		let _ = (|| -> AltoResult<_> {
			...
			unsafe {
				self.ctx.0.dev.0.alto.0.api.alSourceiv(
					self.src,
					efx.AL_AUXILIARY_SEND_FILTER?,
					&mut [0, send, 0] as *mut [sys::ALint; 3] as *mut sys::ALint,
				);
			}
			sends[send as usize] = 0;
			Ok(())
	...
	pub fn clear_aux_effect_slot(&self, slot: sys::ALuint) {
		if let Ok(&ext::ALC_EXT_EFX {
			AL_AUXILIARY_SEND_FILTER: Ok(aasf),
			..
		}) = self.ctx.0.dev.0.exts.ALC_EXT_EFX()
		{
				...
				if *s == slot {
					unsafe {
						self.ctx.0.dev.0.alto.0.api.alSourceiv(
							self.src,
							aasf,
							&mut [0, i as sys::ALint, 0] as *mut [sys::ALint; 3] as *mut sys::ALint,
						);
					}
					*s = 0;
				}
	...
	fn set_air_absorption_factor(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, efx.AL_AIR_ABSORPTION_FACTOR?, value);
		}
		self.ctx.get_error()
	}
	...
	fn set_room_rolloff_factor(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, efx.AL_ROOM_ROLLOFF_FACTOR?, value);
		}
		self.ctx.get_error()
	}
	...
	fn set_cone_outer_gainhf(&self, value: f32) -> AltoResult<()> {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alSourcef(self.src, efx.AL_CONE_OUTER_GAINHF?, value);
		}
		self.ctx.get_error()
	}
	...
	fn direct_filter_gainhf_auto(&self) -> bool {
		...
		.unwrap_or(sys::AL_TRUE as sys::ALint)
			== sys::AL_TRUE as sys::ALint
	}
	...
	fn drop(&mut self) {
		...
		unsafe {
			self.ctx
				...
				.0
				.api
				.alDeleteSources(1, &mut self.src as *mut sys::ALuint);
		}
	}
}
...
impl StaticSource {
		...
		let mut src = 0;
		{
			...
			unsafe {
				ctx.0
					...
					.0
					.api
					.alGenSources(1, &mut src as *mut sys::ALuint);
			}
			ctx.get_error()?;
		}
		let sends = iter::repeat(0)
			.take(ctx.0.dev.0.max_aux_sends() as usize)
			.collect();
		Ok(StaticSource {
			src: Arc::new(SourceInner {
				ctx: ctx,
				src: src,
				sends: Mutex::new(sends),
			}),
			buf: None,
		})
	}
	...
	pub fn set_buffer(&mut self, buf: Arc<Buffer>) -> AltoResult<()> {
...

		{
			...
			unsafe {
				self.src.ctx.0.dev.0.alto.0.api.alSourcei(
					self.src.src,
					sys::AL_BUFFER,
					buf.buf as sys::ALint,
				);
			}
		}
	...
	pub fn clear_buffer(&mut self) {
		{
			...
			unsafe {
				self.src
					...
					.0
					.api
					.alSourcei(self.src.src, sys::AL_BUFFER, 0);
			}
		}
	...
	pub fn looping(&self) -> bool {
		...
		unsafe {
			self.src
				...
				.0
				.api
				.alGetSourcei(self.src.src, sys::AL_LOOPING, &mut value);
		}
		value == sys::AL_TRUE as sys::ALint
	}
...
unsafe impl Source for StaticSource {
	...
	fn set_max_gain(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_max_gain(value)
	}

	#[inline]
	fn reference_distance(&self) -> f32 {
		self.src.reference_distance()
	}
	#[inline]
	fn set_reference_distance(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_reference_distance(value)
	}

	#[inline]
	fn rolloff_factor(&self) -> f32 {
		self.src.rolloff_factor()
	}
	#[inline]
	fn set_rolloff_factor(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_rolloff_factor(value)
	}

	#[inline]
	fn max_distance(&self) -> f32 {
		self.src.max_distance()
	}
	#[inline]
	fn set_max_distance(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_max_distance(value)
	}
	...
	fn set_direction<V: Into<[f32; 3]>>(&mut self, value: V) -> AltoResult<()> {
		self.src.set_direction(value)
	}

	#[inline]
	fn cone_inner_angle(&self) -> f32 {
		self.src.cone_inner_angle()
	}
	#[inline]
	fn set_cone_inner_angle(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_cone_inner_angle(value)
	}

	#[inline]
	fn cone_outer_angle(&self) -> f32 {
		self.src.cone_outer_angle()
	}
	#[inline]
	fn set_cone_outer_angle(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_cone_outer_angle(value)
	}

	#[inline]
	fn cone_outer_gain(&self) -> f32 {
		self.src.cone_outer_gain()
	}
	#[inline]
	fn set_cone_outer_gain(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_cone_outer_gain(value)
	}
	...
	fn set_sec_offset(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_sec_offset(value)
	}

	#[inline]
	fn sample_offset(&self) -> sys::ALint {
		self.src.sample_offset()
	}
	#[inline]
	fn set_sample_offset(&mut self, value: sys::ALint) -> AltoResult<()> {
		self.src.set_sample_offset(value)
	}

	#[inline]
	fn byte_offset(&self) -> sys::ALint {
		self.src.byte_offset()
	}
	#[inline]
	fn set_byte_offset(&mut self, value: sys::ALint) -> AltoResult<()> {
		self.src.set_byte_offset(value)
	}

	#[inline]
	fn soft_sec_offset_latency(&self) -> AltoResult<(f64, f64)> {
		self.src.soft_sec_offset_latency()
	}

	#[inline]
	fn soft_sample_frac_offset_latency(&self) -> AltoResult<(i32, i32, i64)> {
		self.src.soft_sample_offset_frac_latency()
	}

	#[inline]
	fn soft_sec_length(&self) -> AltoResult<f32> {
		self.src.soft_sec_length()
	}

	#[inline]
	fn soft_sample_length(&self) -> AltoResult<sys::ALint> {
		self.src.soft_sample_length()
	}

	#[inline]
	fn soft_byte_length(&self) -> AltoResult<sys::ALint> {
		self.src.soft_byte_length()
	}

	#[inline]
	fn soft_direct_channels(&self) -> bool {
		self.src.soft_direct_channels()
	}
	#[inline]
	fn set_soft_direct_channels(&mut self, value: bool) -> AltoResult<()> {
		self.src.set_soft_direct_channels(value)
	}

	#[inline]
	fn distance_model(&self) -> DistanceModel {
		self.src.distance_model()
	}
	#[inline]
	fn set_distance_model(&mut self, value: DistanceModel) -> AltoResult<()> {
		self.src.set_distance_model(value)
	}

	#[inline]
	fn soft_spatialization(&self) -> SoftSourceSpatialization {
		self.src.soft_spatialization()
	}
	#[inline]
	fn set_soft_spatialization(&mut self, value: SoftSourceSpatialization) -> AltoResult<()> {
		self.src.set_soft_spatialization(value)
	}

	#[inline]
	fn soft_resampler(&self) -> AltoResult<sys::ALint> {
		self.src.soft_resampler()
	}
	#[inline]
	fn set_soft_resampler(&mut self, value: sys::ALint) -> AltoResult<()> {
		self.src.set_soft_resampler(value)
	}

	#[inline]
	fn stereo_angles<V: From<[f32; 2]>>(&self) -> AltoResult<V> {
		self.src.stereo_angles()
	}
	#[inline]
	fn set_stereo_angles<V: Into<[f32; 2]>>(&mut self, value: V) -> AltoResult<()> {
		self.src.set_stereo_angles(value)
	}
	...
	#[inline]
	fn set_radius(&self, value: f32) -> AltoResult<()> {
		self.src.set_radius(value)
	}

	#[inline]
	fn set_direct_filter<F: Filter>(&mut self, value: &F) -> AltoResult<()> {
		self.src.set_direct_filter(value)
	}
	#[inline]
	fn clear_direct_filter(&mut self) {
		self.src.clear_direct_filter()
	}

	#[inline]
	fn set_aux_send(&mut self, send: sys::ALint, slot: &mut AuxEffectSlot) -> AltoResult<()> {
		SourceInner::set_aux_send(&self.src, send, slot)
	}
	#[inline]
	fn set_aux_send_filter<F: Filter>(
		&mut self,
		...
		SourceInner::set_aux_send_filter(&self.src, send, slot, filter)
	}
	#[inline]
	fn clear_aux_send(&mut self, send: sys::ALint) {
		self.src.clear_aux_send(send)
	}

	#[inline]
	fn air_absorption_factor(&self) -> f32 {
		self.src.air_absorption_factor()
	}
	#[inline]
	fn set_air_absorption_factor(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_air_absorption_factor(value)
	}

	#[inline]
	fn room_rolloff_factor(&self) -> f32 {
		self.src.room_rolloff_factor()
	}
	#[inline]
	fn set_room_rolloff_factor(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_room_rolloff_factor(value)
	}

	#[inline]
	fn cone_outer_gainhf(&self) -> f32 {
		self.src.cone_outer_gainhf()
	}
	#[inline]
	fn set_cone_outer_gainhf(&mut self, value: f32) -> AltoResult<()> {
		self.src.set_cone_outer_gainhf(value)
	}

	#[inline]
	fn direct_filter_gainhf_auto(&self) -> bool {
		self.src.direct_filter_gainhf_auto()
	}
	#[inline]
	fn set_direct_filter_gainhf_auto(&mut self, value: bool) -> AltoResult<()> {
		self.src.set_direct_filter_gainhf_auto(value)
	}
}
...
impl StreamingSource {
	...
	pub fn buffers_processed(&self) -> sys::ALint {
		...
		let mut value = 0;
		unsafe {
			self.src.ctx.0.dev.0.alto.0.api.alGetSourcei(
				self.src.src,
				sys::AL_BUFFERS_PROCESSED,
				&mut value,
			);
		}
		value
	...
	pub fn queue_buffer(&mut self, buf: Buffer) -> AltoResult<()> {
		{
			...
			unsafe {
				self.src
					...
					.0
					.api
					.alSourceQueueBuffers(self.src.src, 1, &buf.buf);
			}
		}
	...
	pub fn unqueue_buffer(&mut self) -> AltoResult<Buffer> {
		{
			...
			unsafe {
				self.src
					...
					.0
					.api
					.alSourceUnqueueBuffers(self.src.src, 1, &mut buf);
			}
