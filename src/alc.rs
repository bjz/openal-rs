...
impl Alto {
	...
	fn check_version(&self, dev: *mut sys::ALCdevice) -> AltoResult<()> {
		...
		unsafe {
			self.0
				.api
				.alcGetIntegerv(dev, sys::ALC_MAJOR_VERSION, 1, &mut major);
		}
		let mut minor = 0;
		unsafe {
			self.0
				.api
				.alcGetIntegerv(dev, sys::ALC_MINOR_VERSION, 1, &mut minor);
		}

		if (major == 1 && minor >= 1) || (dev == ptr::null_mut() && major == 0 && minor == 0)
		// Creative's buggy router DLL won't report a version until you open a device
		{
			Ok(())
		} else {
	...
	pub fn raw_api(&self) -> &sys::AlApi {
		&self.0.api
	}

	/// `alcGetString(ALC_DEFAULT_DEVICE_SPECIFIER)`
	pub fn default_output(&self) -> Option<CString> {
		let spec = if let Ok(ext::ALC_ENUMERATE_ALL_EXT {
			ALC_DEFAULT_ALL_DEVICES_SPECIFIER: Ok(dads),
			..
		}) = self.0.exts.ALC_ENUMERATE_ALL_EXT
		{
			...
			unsafe {
				self.0
					.api
					.alcGetString(ptr::null_mut(), sys::ALC_DEFAULT_DEVICE_SPECIFIER)
			}
		};
	...
	pub fn default_capture(&self) -> Option<CString> {
		let spec = unsafe {
			self.0
				.api
				.alcGetString(ptr::null_mut(), sys::ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER)
		};
	...
	pub fn enumerate_outputs(&self) -> Vec<CString> {
		let spec = if let Ok(ext::ALC_ENUMERATE_ALL_EXT {
			ALC_ALL_DEVICES_SPECIFIER: Ok(ads),
			..
		}) = self.0.exts.ALC_ENUMERATE_ALL_EXT
		{
			...
			unsafe {
				self.0
					.api
					.alcGetString(ptr::null_mut(), sys::ALC_DEVICE_SPECIFIER)
			}
		};
	...
	pub fn enumerate_captures(&self) -> Vec<CString> {
		let spec = unsafe {
			self.0
				.api
				.alcGetString(ptr::null_mut(), sys::ALC_CAPTURE_DEVICE_SPECIFIER)
		};
		Alto::parse_enum_spec(spec as *const u8)
	}
	...
	fn parse_enum_spec(spec: *const u8) -> Vec<CString> {
		...
		specs.extend(
			unsafe { ::std::slice::from_raw_parts(spec as *const u8, i as usize) }
				.split(|c| *c == 0)
	...
	pub fn open(&self, spec: Option<&CStr>) -> AltoResult<OutputDevice> {
		...
		let dev = unsafe {
			self.0
				.api
				.alcOpenDevice(spec.as_ref().map(|s| s.as_ptr()).unwrap_or(ptr::null()))
		};
	...
	pub fn open_loopback<F: LoopbackFrame>(
		...
		let spec = spec.map(|s| s.to_owned()); //.or_else(|| self.default_output());
		let dev = unsafe {
			asl.alcLoopbackOpenDeviceSOFT?(spec.as_ref().map(|s| s.as_ptr()).unwrap_or(ptr::null()))
		};
	...
	pub fn open_capture<F: StandardFrame>(
		...
		let dev = unsafe {
			self.0.api.alcCaptureOpenDevice(
				spec.as_ref().map(|s| s.as_ptr()).unwrap_or(ptr::null()),
				freq,
				F::format().into_raw(None)?,
				len,
			)
		};
...
impl DeviceInner {
	...
	pub fn connected(&self) -> AltoResult<bool> {
		let mut value = 0;
		unsafe {
			self.alto.0.api.alcGetIntegerv(
				self.dev,
				self.exts.ALC_EXT_DISCONNECT()?.ALC_CONNECTED?,
				1,
				&mut value,
			);
		}
	...
	pub fn enumerate_soft_hrtfs(&self) -> Vec<CString> {
		...
		let _ = (|| -> AltoResult<_> {
			...
			unsafe {
				self.alto.0.api.alcGetIntegerv(
					self.dev,
					ash.ALC_NUM_HRTF_SPECIFIERS_SOFT?,
					1,
					&mut value,
				);
			}
			...
			for i in 0..value {
				unsafe {
					let spec =
						ash.alcGetStringiSOFT?(self.dev, ash.ALC_HRTF_SPECIFIER_SOFT?, i) as *mut _;
					spec_vec.push(
						self.alto
							.get_error(self.dev)
							.map(|_| CStr::from_ptr(spec).to_owned())?,
					);
				}
			}
		...
		(|| -> AltoResult<_> {
			...
			unsafe {
				self.alto
					.0
					.api
					.alcGetIntegerv(self.dev, ash.ALC_HRTF_STATUS_SOFT?, 1, &mut value);
			}
			self.alto.get_error(self.dev).and_then(|_| match value {
				...
				s if s == ash.ALC_HRTF_HEADPHONES_DETECTED_SOFT? => {
					Ok(SoftHrtfStatus::HeadphonesDetected)
				}
				s if s == ash.ALC_HRTF_UNSUPPORTED_FORMAT_SOFT? => {
					Ok(SoftHrtfStatus::UnsupportedFormat)
				}
	...
	pub fn max_aux_sends(&self) -> sys::ALCint {
		...
		let _ = (|| -> AltoResult<_> {
			unsafe {
				self.alto.0.api.alcGetIntegerv(
					self.dev,
					self.exts.ALC_EXT_EFX()?.ALC_MAX_AUXILIARY_SENDS?,
					1,
					&mut value,
				);
			}
...
impl Drop for DeviceInner {
	fn drop(&mut self) {
		unsafe {
			self.alto.0.api.alcCloseDevice(self.dev);
		}
	}
}
...
impl OutputDevice {
		...
		if let Some(attrs) = attrs {
			...
			if let Ok(ash) = self.0.exts.ALC_SOFT_HRTF() {
				if let Some(hrtf) = attrs.soft_hrtf {
					attrs_vec.extend(&[
						ash.ALC_HRTF_SOFT?,
						if hrtf { sys::ALC_TRUE } else { sys::ALC_FALSE } as sys::ALCint,
					]);
				}
			...
			if let Ok(asol) = self.0.exts.ALC_SOFT_output_limiter() {
				if let Some(lim) = attrs.soft_output_limiter {
					attrs_vec.extend(&[
						asol.ALC_OUTPUT_LIMITER_SOFT?,
						if lim { sys::ALC_TRUE } else { sys::ALC_FALSE } as sys::ALCint,
					]);
				}
			}
	...
	pub fn new_context(&self, attrs: Option<ContextAttrs>) -> AltoResult<Context> {
		...
		let ctx = unsafe {
			self.0.alto.0.api.alcCreateContext(
				self.0.dev,
				attrs_vec
					.map(|a| a.as_slice().as_ptr())
					.unwrap_or(ptr::null()),
			)
		};
		if ctx == ptr::null_mut() {
...
unsafe impl DeviceObject for OutputDevice {
	...
	fn is_extension_present(&self, ext: ext::Alc) -> bool {
		self.0.is_extension_present(ext)
	}
	#[inline]
	fn enumerate_soft_hrtfs(&self) -> Vec<CString> {
		self.0.enumerate_soft_hrtfs()
	}
	#[inline]
	fn soft_hrtf_status(&self) -> SoftHrtfStatus {
		self.0.soft_hrtf_status()
	}
	#[inline]
	fn soft_output_limiter(&self) -> bool {
		self.0.soft_output_limiter()
	}
	#[inline]
	fn max_aux_sends(&self) -> sys::ALCint {
		self.0.max_aux_sends()
	}
	#[inline]
	fn to_device(&self) -> Device {
		Device(self.0.clone())
...
impl<F: LoopbackFrame> LoopbackDevice<F> {
	...
	pub fn new_context(
		...
		let ctx = unsafe {
			self.0
				...
				.0
				.api
				.alcCreateContext(self.0.dev, attrs_vec.as_slice().as_ptr())
		};
		if ctx == ptr::null_mut() {
			match self.0.alto.get_error(self.0.dev) {
				Ok(..) => Err(AltoError::NullError),
	...
	pub fn soft_render_samples<R: AsBufferDataMut<F>>(&mut self, mut data: R) -> usize {
		...
		let len = cmp::min(
			size / mem::size_of::<F>(),
			sys::ALCsizei::max_value() as usize,
		);
		if len == 0 {
			return 0;
		}
		...
		unsafe {
			asl.alcRenderSamplesSOFT.unwrap()(self.0.dev, data, len as sys::ALCsizei);
		}
...
impl<F: StandardFrame> Capture<F> {
	...
	#[inline]
	pub fn as_raw(&self) -> *mut sys::ALCdevice {
		self.dev
	}
	...
	pub fn start(&mut self) {
		unsafe {
			self.alto.0.api.alcCaptureStart(self.dev);
		}
	}
	...
	pub fn stop(&mut self) {
		unsafe {
			self.alto.0.api.alcCaptureStop(self.dev);
		}
	}
	...
	pub fn samples_len(&self) -> sys::ALCint {
		...
		unsafe {
			self.alto
				.0
				.api
				.alcGetIntegerv(self.dev, sys::ALC_CAPTURE_SAMPLES, 1, &mut samples);
		}
		samples
	}
	...
	pub fn capture_samples<R: AsBufferDataMut<F>>(&mut self, mut data: R) -> AltoResult<usize> {
		...
		unsafe {
			self.alto
				.0
				.api
				.alcCaptureSamples(self.dev, data, len as sys::ALCsizei);
		}
		Ok(len as usize)
	}
}
...
impl<F: StandardFrame> Drop for Capture<F> {
	fn drop(&mut self) {
		unsafe {
			self.alto.0.api.alcCaptureCloseDevice(self.dev);
		}
	}
}
